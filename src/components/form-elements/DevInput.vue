...
<template>
  <div class="relative">
    <i v-if="icon" :class="icon"
      class="absolute left-4 transform translate-y-[0.843rem] text-[16px] text-brandSoftGrey"></i>
      ...
      <input
        ...
        v-bind="field" :class="[
          !meta.valid && errorMessage
            ...
            : null,
          paddingLeft
        ]" :name="name" :type="type" @input="(e) => sendValues(e)" autocomplete="off" :placeholder="placeholder" />

    </Field>
    <ErrorMessage class="absolute right-4 top-4 text-brandRed text-[12px] animate__animated animate__wobble"
      :name="name" />
  </div>
</template>
